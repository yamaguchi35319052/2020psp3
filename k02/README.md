...
## ソースコードの説明
79行目～　新しい要素のデータ領域を確保し、そこにデータを代入し、リストに要素を挿入する。新たに確保したデータ領域の先頭（pNode）をreturnで返す。

107行目～　入力されたIDを検索する。入力されたIDと比較するIDが一致していれば探索を終了し、一致していなければ一つ進めて一致するまで繰り返す。


## 出力結果

```
PS C:\Users\akira\Desktop\2020psp3\k02> ./k02
