...
## ソースコードの説明
75行目～　動的計画法で1000円で最大取得可能なカロリーを求める。メニューを一つずつ容量の少ないところから順番に、メニューが入りきれない場合は一つ上の値をコピーし、メニューが入る場合は入れ替えた場合のカロリーの値(ｎ)とと入れ替えなかった場合のカロリーの値(nap_value)を比較し、大きいほうの値を入れる。


## 出力結果

```
PS C:\Users\akira\Desktop\2020psp3\k06> ./k06
MAXcalorie = 1796
```
